...
class MixedEnv(gym.Env):
    ...
    def step(self, action):
        assert self.current_env is not None, "Environment must be reset before step."
...
def train(env_paras, device, save_dir, timesteps_real_per_round = 500, timesteps_fc_per_round = 20000, epoch_per_round = 100, rounds_num = 30, batch_size_env_model = 128):
    ...
    for i in range(CLIENTS_NUM):
        init_envs.append( TimeLimit(PendulumEnv(g=env_paras[i]), max_episode_steps=200) )
    
    mixed_env = MixedEnv(envs=init_envs)
    
    # env_theta = [0.1, 0.3, 0.5, 0.7, 0.9]
    real_envs = []
    Clients = []
    for i in range(CLIENTS_NUM):
        real_envs.append( mixed_env )
...
if __name__ == '__main__':
	...
	rounds_num = 30
	batch_size_env_model = 128
	test_dir= "H_Env_Pen_MixedEnv_MBRL_Client=3"
