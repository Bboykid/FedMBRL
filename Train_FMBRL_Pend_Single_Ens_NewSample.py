...
from Client_diff_NewSample import FRLClient
...
def train(env_paras, device, save_dir, timesteps_real_per_round = 500, timesteps_fc_per_round = 20000, epoch_per_round = 100, rounds_num = 30, batch_size_env_model = 128):
    ...
    #     Server.learn(timesteps_real_per_round = 10000)
        ...
        for client_idx in range(CLIENTS_NUM):
            mean_reward, std_reward = evaluate_policy(Global_RL, real_envs[client_idx], n_eval_episodes=30)
...
if __name__ == '__main__':
 ...
 rounds_num = 30
 batch_size_env_model = 128 
 test_dir= "H_Env_Env3/EnsNum3_r30_epoch20_ts500_NewSample"
...
#  env_paras = [13.0]
 ...
 for exper_idx in range(exper_num):
  for env_idx in range(len(env_paras)):
   ...
   rewards_log =train(env_para, device, test_dir, timesteps_real_per_round, timesteps_fc_per_round, epoch_per_round, rounds_num, batch_size_env_model)
   np.save(test_dir + "/" + str(exper_idx) + "_SngleModel_env_+"+str(env_idx) + "_reward_logs.npy", rewards_log)