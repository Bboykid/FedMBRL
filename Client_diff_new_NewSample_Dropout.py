...
from PredictionModel import PredictionModel, DropoutMLP, mc_dropout_inference
from Utils import *
...
class FRLClient():
    ...
    def __init__(
        ...
        self.model = DropoutMLP(self.obs_size, self.action_size, hidden_size).to(device)
        self.agent = agent
    ...
    with torch.no_grad():
        ...
        for i in range(round((len(y) / batch_size) + 1)):
            ...
            for k in range(min(batch_size, len(test_X))):
                ...
                mean_pred, var_pred = mc_dropout_inference(model, test_X[k].float(), mc_iterations=20)
                
                # loss += loss_fn(pred.to(torch.float32), test_y[k].to(torch.float32))
                loss = loss_fn(mean_pred, test_y[k].float())
                loss_sum += loss.item()
