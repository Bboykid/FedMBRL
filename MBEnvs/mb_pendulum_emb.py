...
class MB_PendulumEnv(gym.Env):
    ...
    * v0: Initial versions release
    ...
    def __init__(self, models, env_embs, device):
        ...
        self.clock = None
        self.isopen = True
        self.env_embs = env_embs
        
        ori_high = np.array([1.0, 1.0, self.max_speed], dtype=np.float32)
        high = np.array([1.0, 1.0, self.max_speed, 20.0], dtype=np.float32)
        ...
        self.ori_observation_space = spaces.Box(low=-ori_high, high=ori_high, dtype=np.float32)
        '''
        	env models
        '''
        self.models = models
    ...
    def step(self, u):
        ...
        obs_emb = np.concatenate((next_obs, self.emb)).astype(np.float32)

        # truncation=False as the time limit is handled by the `TimeLimit` wrapper added during `make`
        return obs_emb, reward, terminal, False, {}
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        select one model
        '''
        self.model_index = np.random.randint(0, len(self.models))
        self.cur_model = self.models[self.model_index]
        self.emb = self.env_embs[self.model_index]
        obs = self._get_obs()
        self.obs = obs
        
        obs_emb = np.concatenate((obs, self.emb)).astype(np.float32)
        return obs_emb, {}
    ...
    def _get_obs_by_model(self, obs, action):
        ...
        obs_clipped = np.clip(obs, self.ori_observation_space.low, self.ori_observation_space.high)
        return obs_clipped
