...
class MB_PendulumEnv(gym.Env):
    ...
    * v0: Initial versions release
    ...
    def _get_obs_by_model(self, obs, action):
        ...
        mean, logvar = self.cur_model.forward(input_d)
...
def infer(model, observation, action, device):
    model.eval()
    with torch.no_grad():
        # Prepare the input tensor
        input_tensor = torch.cat((observation, action), dim=-1).to(device)
        
        # Get the mean and log variance from the model
