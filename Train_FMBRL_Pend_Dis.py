...
def train(env_paras, device, save_dir, timesteps_real_per_round = 500, timesteps_fc_per_round = 20000, epoch_per_round = 100, rounds_num = 30, batch_size_env_model = 128):
    ...
    agent_server = SB3Agent(Global_RL)
    Server = FRLServer(MB_env, agent_server, lr = 3e-4, hidden_size = 256, device = device)
    ...
    #     Server.update_env_models(env_models)
        ...
        print('------------------------------')
        print("server distillation start:")
        MB_env = TimeLimit(MB_PendulumEnv(env_models,device), max_episode_steps = 200)
        Server.agent.policy_net = Global_RL
        Server.set_env(MB_env)
        Server.learn_dis_model(timesteps_real_per_round * 10, epoch_per_round, batch_size_env_model)
        dis_model = Server.get_dis_model()
        MB_env = TimeLimit(MB_PendulumEnv([dis_model],device), max_episode_steps = 200)
    ...
    #     Server.learn(timesteps_real_per_round = 10000)
        ...
        for client_idx in range(CLIENTS_NUM):
            Clients[client_idx].model.load_state_dict(Server.get_dis_model_params())
        ...
        for client_idx in range(CLIENTS_NUM):
            mean_reward, std_reward = evaluate_policy(Global_RL, real_envs[client_idx], n_eval_episodes=10)
...
if __name__ == '__main__':
	...
	rounds_num = 50
	batch_size_env_model = 128
	test_dir= "H_Env3_Dis_EnsNum3"
	env_paras = [13, 13.0, 13.0]
